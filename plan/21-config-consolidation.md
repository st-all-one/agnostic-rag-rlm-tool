...
- `arags-server/src/config.rs::ServerConfig` e `arags-cli/src/config.rs::Config` **lêem os mesmos
  arquivos** (`.arags/config.toml` local e `~/.arags/config.toml` global) mas desserializam
  structs diferentes.
...
- `llm` aparece **duas vezes** com tipos diferentes: `ServerConfig.llm` (server, será removido no
  019) e `Config.llm` (`arags_llm::LlmConfig`, user AI). `agent.max_depth/max_nodes` são do run
  (serão removidos).
- `Config::load()` lê `data_dir()/config.toml` (`~/.arags/config.toml`) — sem noção de config
...
2. **User global** (`~/.arags/arags.toml`) — identidade + IA do usuário + alvo do server.
3. **User local** (`.arags.toml`, na raiz do repo, **gitignored**) — overrides por projeto, com
   **fallback granular** para o global.

> **Mudança de nomenclatura (supresa 019/018):** adota-se `~/.arags/arags.toml` (global) e
> `.arags.toml` (local) **em vez de** `~/.arags/config.toml` / `.arags/config.toml`. O server config
...
>   (ex.: `./server.toml:/etc/arags/server.toml`). Dentro do container fica em
>   `/etc/arags/server.toml` (ou `ARAGS_SERVER_CONFIG`).
...
> - **Break total, sem retrocompatibilidade.** O legado `~/.arags/config.toml` / `.arags/config.toml`
>   é **ignorado** (não há janela de transição).
> - **Auth é global**, vinculado ao server. `[auth]` existe **só** em `~/.arags/arags.toml` (global);
>   o `.arags.toml` local **não** pode definir `auth` (e o merge ignora se presente).
...
- **User config em 2 escopos**: global (`~/.arags/arags.toml`) e local (`.arags.toml`), com merge
...
- Local `.arags.toml` é gerado por `arags init`, **auto-adicionado ao `.gitignore`**, replica o
...
- **Zero config de dados no client + sem modo offline**: o client é um **puro gRPC client**
  ...
  local, **não** embeda, **não** busca localmente, **não** chunka. Tudo depende do `arags-server`.
- Remover `[llm]` do server e `agent` (run) da config do client.

## Non-goals

- Não criar UI/editor de config.
- **Não há transição/retrocompatibilidade**: o legado `~/.arags/config.toml` / `.arags/config.toml`
...
|---|---|---|---|
| `server.toml` (docker: `/etc/arags/server.toml` ou `ARAGS_SERVER_CONFIG`) | `arags-server` | data plane completo (ver schema) | não (TLS keys são paths) |
| `~/.arags/arags.toml` | `arags-cli` | `auth` + `llm`(user) + `server.addr` | **sim** (refresh_token) |
| `.arags.toml` (raiz do repo, gitignored) | `arags-cli` | overrides por projeto | não (cai no global p/ auth) |

O client faz `merge(global, local)` e usa o resultado; o server **não lê** `.arags.toml` nem
`~/.arags/arags.toml`. O client **não lê** `server.toml`.
...
> (`docker compose`/`docker -v`: `./server.toml:/etc/arags/server.toml`). Dentro do container,
> lido de `/etc/arags/server.toml` (ou `ARAGS_SERVER_CONFIG`). **Não** é gerado pelo client nem
> vive no `~/.arags` do client.

```toml
# ── Servir / receber ───────────────────────────────────────────────
listen_addr = "0.0.0.0:50051"        # (env ARAGS_SERVER_ADDR sobrescreve)
tls_cert = "/etc/arags/tls/server.crt"   # optional → habilita TLS
tls_key  = "/etc/arags/tls/server.key"   # optional
# mtls_ca = "/etc/arags/tls/ca.crt"       # optional → exige client cert

# ── Armazenamento / dados ─────────────────────────────────────────
data_dir = "/var/lib/arags"           # (env ARAGS_DATA_DIR sobrescreve)
pool_size = 4
...
### Schema: `~/.arags/arags.toml` (global, user)

```toml
[auth]
username = "dev1"
refresh_token = "<token gerado por arags-server admin create-refresh>"
...
### Schema: `.arags.toml` (local, projeto, gitignored)
...
`auth` do global. `arags init` gera apenas `[project]` (+ overrides desejados) e **não** grava o
...
`EffectiveUserConfig = merge(global ~/.arags/arags.toml, local .arags.toml)`:
...
Com o modo offline removido, o `arags-cli` deixa de ser um data plane local. **Tudo** passa pelo
`arags-server` (gRPC/TLS), exceto a síntese/summarize que usa o **LLM do próprio usuário** localmente
(plan 017/020: `auth` + `llm` vêm do `~/.arags/arags.toml`).

- **Removidos do client**: `dispatch/local.rs` (branch local), `arags_storage::Storage::open`
...
- **Comandos sobreviventes viram chamadas gRPC puras** (server é a fonte de verdade):
  ...
  - `persist` → `GetAnswerById` (server) + LLM do usuário local (summarize) + escrita do
    `wiki/...md` **local** (o `.arags.toml`/`~/.arags` do client só guarda config, não DB).
  - `history` → `GetHistory` (server, por `username`).
  - `init` → gera `.arags.toml` + dispara `index`.
- O `--server`/endereço vem de `user_config` (`server.addr` global ou override local);
  `ARAGS_SERVER_ADDR` ainda funciona como override de env (equivalente a setar `server.addr`).
...
- **D1 — `server.toml` é arquivo do HOST, montado no container.** Não vive "dentro" do repo nem é
  ...
  `/etc/arags/server.toml` (dentro do container; `ARAGS_SERVER_CONFIG` sobrescreve o caminho
  interno). `ARAGS_SERVER_ADDR`/`ARAGS_DATA_DIR` continuam como overrides de env. O arquivo é
  disjunto do `~/.arags/arags.toml` do client.
...
- **D3 — Modo offline REMOVIDO.** Tudo depende do `arags-server`. Quem quiser "offline" sobe o
  ...
  são **eliminados**.
- **D4 — Sem transição.** Break total, sem retrocompatibilidade. O legado `~/.arags/config.toml` /
  `.arags/config.toml` é **ignorado** (não há fallback nem warning).
- **D5 — Auth global, vinculado ao server.** `[auth]` existe **só** em `~/.arags/arags.toml`
  (global). O `.arags.toml` local não define `auth`; o merge o ignora se presente. Identidade é
  única e global.
...
| `[project].name` / `[project].ignore` | escopo do repo (não dado) | **`.arags.toml` local** (client, só p/ descobrir arquivos) |
| `auth.username` / `refresh_token` | identidade user | **`~/.arags/arags.toml` global** |
| `llm.backends` (IA do user) | consumo de IA | **`~/.arags/arags.toml` global** (+ override local) |
...
|---|---|---|
| `server.toml` schema + load (host mount) | `arags-server` | `src/config.rs` (rework: remover `llm`, add `embedder`/`search`/`maintenance`/`tls.mtls_ca`; `load` de `ARAGS_SERVER_CONFIG` default `/etc/arags/server.toml`) |
| Remover `[llm]` server + `build_llm` (019) | `arags-server` | `config.rs`, `lifecycle.rs`, `state.rs` |
| User config 2-escopos + merge granular (auth só global) | `arags-cli` | `src/user_config.rs` (novo); rework `src/config.rs` (apenas auth/llm/server) |
| `arags init` gera `.arags.toml` + gitignore | `arags-cli` | `src/commands/init.rs` (019) + `user_config` |
...
| Client lê `server.addr` do merge | `arags-cli` | `src/client.rs`, `dispatch/server.rs`, `auth_client.rs` |
| Index protocolo: client manda **cru**, server chunka (D2) | `arags-proto`+`arags-server`+`arags-cli` | `proto` (`IndexFile` texto cru), `grpc/index.rs` (server chunka/embeda), `commands/index.rs` (019) |
| Admin print path update | `arags-server` | `src/admin.rs` (mensagem → `~/.arags/arags.toml`) |
| Break total: ignorar legacy `config.toml` | `arags-cli` | `user_config::load` **não** lê `~/.arags/config.toml`/`.arags/config.toml` (sem fallback) |
...
1. **Server config rework**: `ServerConfig` recebe `embedder`/`search`/`maintenance`/`mtls_ca`;
   remove `llm` + `build_llm`; `load()` de `ARAGS_SERVER_CONFIG` (default `/etc/arags/server.toml`),
   mantendo `ARAGS_SERVER_ADDR`/`ARAGS_DATA_DIR`.
2. **Client user_config**: novo `user_config.rs` com struct (auth/llm/server/project) toda
   `Option` + `merge(global, local)` recursivo; `load()` lê `~/.arags/arags.toml` e `.arags.toml`.
3. **Init**: gera `.arags.toml` mínimo (`[project]`) e faz `append` de `.arags.toml` ao `.gitignore`
...
4. **Client despido de data-config + modo offline REMOVIDO**: remover `Config.embedding`/
   ...
   `arags_storage::Storage::open` local, `embedding.rs`, busca/contexto locais e vector store
...
6. **Wire**: `client.rs`/`auth_client.rs`/`dispatch/server.rs` consomem `user_config` (addr +
   auth + llm); `ARAGS_SERVER_ADDR` continua como override de env.
7. **Docs**: `install.sh`/`docker-compose`/`README` documentam `server.toml` (host mount),
   `~/.arags/arags.toml` e `.arags.toml`; `arags-server admin create-refresh` aponta para
   `~/.arags/arags.toml [auth]`. Sem nota de transição.
...
- `test_server_config_loads_from_arags_server_config_env` (default `/etc/arags/server.toml`).
...
- `test_init_creates_local_arags_toml_and_gitignores` (`.arags.toml` em `.gitignore`).
- `test_init_does_not_write_auth_to_local` (refresh_token fica só no global).
- `test_client_uses_merged_server_addr`.
- `test_legacy_config_toml_ignored` (break total: `~/.arags/config.toml`/`.arags/config.toml`
  antigos são **ignorados**, não lidos).
- `test_auth_only_global` (merge ignora `[auth]` presente em `.arags.toml` local).
- `test_server_and_user_config_files_disjoint` (server não lê `~/.arags/arags.toml`; client não lê
  `server.toml`).
...
| Risco | Mitigação |
|---|---|
| Colisão histórica `[server]` (addr vs listen_addr) | arquivos separados (`server.toml` vs `arags.toml`); sem sobreposição |
| Break total sem transição | operadores devem reescrever configs (`server.toml` + `~/.arags/arags.toml`); documentar como *breaking change* no CHANGELOG; sem auto-migração |
...
| `.arags.toml` commitado por engano | `arags init` garante gitignore; documentar |

---

## Relação com 019/017/018

- **019 (remoção legado + CLI):** `arags init` (B) e `arags memory`/`persist` (C/D) consomem
  `user_config`; `maintenance` (C.1) é configurado aqui em `[maintenance]`. As referências a
  `~/.arags/config.toml`/`.arags/config.toml` no 019 são **supersedidas** por este plano
  (`~/.arags/arags.toml` global, `.arags.toml` local, `server.toml` do server).
...
- **018 (auth):** `auth.username`/`refresh_token` migram de `config.toml [auth]` para
  `~/.arags/arags.toml [auth]`; semântica de token inalterada.
