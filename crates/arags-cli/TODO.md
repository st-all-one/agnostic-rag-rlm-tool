...
> O `arags-cli` agora é um **cliente gRPC puro** (sem modo local), e os
...
> `query -qa` e `persist`. A config é 2-escopos (`~/.arags/arags.toml` global +
> `.arags.toml` local). Veja `plan/019-cli-consolidation.md` e
...
- **STATUS:** ⏸️ ADIADO — requer mudanças em `arags-server`/`arags-proto` (cross-crate, fora de escopo). O catch-all agora retorna `bail!` estruturado listando os comandos suportados em modo servidor (search, status, session, run, cost, context).
...
- **Plano:** Plan 03/16 — `arags run` sem `--llm` deve apenas mostrar help ou erro.
...
- **STATUS:** ✅ CONCLUÍDO — `Config.server: ServerSection { addr: Option<String> }` adicionado; `ClientConfig::load()` lê `server.addr` de `.arags/config.toml` / `~/.arags/config.toml`.
