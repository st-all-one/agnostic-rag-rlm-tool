...
use arags_proto::proto::{
    ...
    CompletePendingQaResponse, GetAnswerByIdRequest, GetAnswerByIdResponse, InvalidateCacheRequest,
    InvalidateCacheResponse, InvalidateMode, QueryWithCacheRequest, QueryWithCacheResponse,
    SearchResult, StoreAnswerRequest, StoreAnswerResponse,
};
...
/// (issue `agnostic-rlm-rs-d172`). Prefers a job whose `preferred_user` matches
/// the worker; otherwise the oldest pending job is taken. The server is
/// LLM-free — the volunteer digests locally and stores via `StoreAnswer`.
///
/// # Errors
///
/// Returns `UNAUTHENTICATED` without a session, or `internal` on failure.
...
/// Complete a leased QA re-digest job (issue `agnostic-rlm-rs-d172`). Called
/// after the volunteer persisted the fresh answer through `StoreAnswer`. Returns
/// `ok = false` when the job was not `leased` (e.g. the lease expired).
///
/// # Errors
///
/// Returns `UNAUTHENTICATED` without a session, or `internal` on failure.
