...
- **Arquivo:** `arlm-core/src/token_counter.rs`
- **Problema:** `MODEL_CONTEXT_LIMITS` e `TokenCounter` estão em `arlm-core`, não em `arlm-llm`.
- **Plano:** Plan 02 — `limits.rs` e `token_counter.rs` devem estar em `arlm-llm`.
- **Correção necessária:** Mover `token_counter.rs` para `arlm-llm` ou criar módulo equivalente.
