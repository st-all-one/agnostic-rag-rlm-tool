...
///
/// This struct is the single source of truth that drives [`GenericBackend`].
...
/// that `config.toml` (typically at `~/.arlm/config.toml`) deserializes into.
...
impl std::str::FromStr for LlmConfig {
    type Err = LlmError;
