...
Definições Protobuf e tipos gerados para a comunicação gRPC do arags (cliente ↔ servidor).

## Responsabilidades

- **Contrato gRPC**: fonte única da verdade para a comunicação cliente/servidor via tonic.
- **Schema versionado**: pacote `arags.v1` (versionamento explícito de proto).
...
└── service.proto      # service AragsService (RPCs atuais, inclui ListMemory/GetCache/
                       #   TriggerMaintenance/GetHistory definidos inline)
build.rs               # tonic_build: compila os sub-arquivos + timing/logs
src/lib.rs             # pub mod proto { include!(arags.v1.rs) }; pub use proto::*;
tests/proto_contract.rs# testes de integração validando o contrato gerado
```

## Versionamento

O schema usa `package arags.v1;`. O `lib.rs` faz `include!` do arquivo gerado
`arags.v1.rs` dentro de `mod proto`, preservando os caminhos downstream:
`arags_proto::proto::*`, `arags_proto::proto::arags_service_server::AragsService` e
`arags_proto::proto::arags_service_client::AragsServiceClient`.
...
```
[arags-proto/build] stage=compile_protos duration_ms=<ms> files=<n>
```
