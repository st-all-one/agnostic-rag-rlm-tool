...
Tipos e utilidades compartilhados do `arags` (Agnostic RLM). **Não contém mais
nenhum engine RLM recursivo** — após os planos 017–020, o `arags-core` é uma crate
de biblioteca de suporte usada por `arags-cli` e `arags-server`.

## O que faz

- Define os **tipos de domínio** compartilhados (`types/`: enums, node, input)
  usados em toda a workspace e no contrato gRPC (`arags-proto`).
...
> `arags` é hoje *on-demand* e *server-first*: o servidor é um plano de dados
