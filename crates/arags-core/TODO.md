...
> (planner → solver → synthesizer), que **foi removido** do crate. O `arags-core`
...
- **Problema:** Tool retorna resultado fake — `"Placeholder: in real implementation, this would call arags-search"`.
- **Plano:** Plan 016 — Tools devem ser executáveis de verdade.
- **Status:** `SearchCodeTool` agora usa `Option<Arc<dyn CodeSearch>>` (trait definido em `arags-core`); sem backend retorna mensagem honesta `"search_code not configured: no code-search backend provided"`.
