...
## O que faz
Abstração unificada e **agnóstica a provider** de backends LLM para o `arags`: expõe uma
...
## Dependências
- Internas: nenhuma (crate folha de LLM; consumido por `arags-cli` — o servidor
  `arags-server` é LLM-free, portanto **não** usa `arags-llm`).
...
## Comandos úteis
```bash
CARGO_BUILD_JOBS=4 cargo check -p arags-llm --all-targets
CARGO_BUILD_JOBS=4 cargo clippy -p arags-llm --all-targets
CARGO_BUILD_JOBS=4 cargo test   -p arags-llm
```
