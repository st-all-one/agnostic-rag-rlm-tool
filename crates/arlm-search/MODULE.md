...
## O que faz
Motor de busca híbrida do `arlm`: BM25 (FTS5), busca por entidades (regex + FTS5), busca semântica (usearch via `arlm-storage`), fusão Reciprocal Rank Fusion (RRF, k=60), decay de saliência opcional e montagem de contexto para LLM com token budget.
...
> `arlm-llm`, e o servidor ficou LLM-free também no grafo de dependências.
...
## Dependências
- Internas: `arlm-storage` (metadados + FTS5 + vector store/usearch).
...
## Migrations
- N/A — o schema (incluindo `chunks_fts`, `qa_cache_fts`) vive em `arlm-storage`. `arlm-search` consome as tabelas via `Storage`.
