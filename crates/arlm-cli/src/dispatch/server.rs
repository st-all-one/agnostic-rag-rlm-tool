...
fn run_index(
    ...
    // Combine CLI ignore patterns with the project's `.arlm.toml` ignore list.
...
async fn stream_index_group(
    ...
    let (tx, rx) = mpsc::channel::<arlm_proto::proto::IndexChunk>(32);
    let stream = ReceiverStream::new(rx);
    let response_fut = client.index_project(stream);

    let send_handle = tokio::spawn(async move {
        if tx
            .send(arlm_proto::proto::IndexChunk {
                body: Some(index_chunk::Body::Init(arlm_proto::proto::IndexInit {
                    project,
        ...
        for file in &files {
            ...
            if tx
                .send(arlm_proto::proto::IndexChunk {
                    body: Some(index_chunk::Body::File(arlm_proto::proto::IndexFile {
                        rel_path,
    ...
    let inner: arlm_proto::proto::IndexResponse = response.into_inner();
...
fn render_cache_get(resp: &arlm_proto::proto::GetCacheResponse, format: Format) {
    if format == Format::FullJson {
        let out = crate::output::json::JsonOutput::ok()
            .with_data(serde_json::json!({
...
fn run_init(
    ...
    // global `~/.arlm/arlm.toml`; we never copy it into the local file.
    match cfg.auth() {
        Some(auth) if auth.refresh_token.is_some() => {}
        _ => {
            bail!(
                "no global identity configured. Run `arlm-server admin create-refresh` and \
                 store the token in `~/.arlm/arlm.toml` under `[auth]`."
            );
        }
    }
    ...
    if do_index {
        ...
        println!("Skipping index (--no-index). Run `arlm index` to ingest.");
    }
    Ok(())
}

/// Local `.arlm.toml` shape written by `arlm init`.
...
/// Append `.arlm.toml` to `.gitignore` (idempotent).
