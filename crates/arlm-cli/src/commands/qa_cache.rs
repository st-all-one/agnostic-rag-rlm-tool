...
//!
//! - `arlm query --qa ...`: `QueryWithCache` + client-side digest-once. On a
...
//! - `arlm query --cache-id <id>`: direct `GetAnswerById` lookup (anti-drift).
//! - `arlm cache get|invalidate`: direct cache inspection / admin invalidation.
...
pub fn run_ask(
    ...
    let answer = rt
        .block_on(llm.complete(arlm_llm::CompletionRequest {
