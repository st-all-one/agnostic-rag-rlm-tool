...
//! `#[ignore]`d and gated on `ARLM_MINILM_DIR`. Everything else runs on
//! synthetic fixtures.
...
fn test_minilm_missing_model_file() {
    ...
    let err = arlm_embedding::embedder::MinilmEmbedder::new(
        dir.path(),
...
/// weights. Run with:
/// `ARLM_MINILM_DIR=/models/all-MiniLM-L6-v2 cargo test -- --ignored`
#[test]
#[ignore = "requires real all-MiniLM-L6-v2 weights"]
fn test_minilm_real_weights_semantics() {
    let Some(dir) = std::env::var_os("ARLM_MINILM_DIR").map(std::path::PathBuf::from) else {
        panic!("ARLM_MINILM_DIR not set");
    };
    let embedder = arlm_embedding::embedder::MinilmEmbedder::new(
        &dir,
