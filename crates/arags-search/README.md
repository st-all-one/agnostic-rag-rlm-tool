...
Busca híbrida (BM25 + semântica) com fusão RRF para o arags.

## Responsabilidades

- **BM25**: Busca textual via SQLite FTS5
- **Entity**: Busca por entidades regex (function names, imports)
- **Semantic**: Busca por similaridade via usearch (arags-storage `VectorStore`)
...
> O servidor (`arags-server`) é LLM-free. A busca híbrida (BM25 + semântica + RRF)
> roda inteiramente no servidor; o rerank/LLM só ocorre no cliente (`arags-cli`) em
...
├── semantic.rs     # SemanticSearch via usearch (arags-storage)
├── hybrid/
...
```rust
use arags_search::HybridSearch;
use arags_search::types::{SearchOptions, SearchTier};
