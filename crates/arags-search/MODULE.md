...
- `src/hybrid/rrf.rs` — `rrf_score` + `rrf_fuse` (matemática pura de fusão, sem I/O). **plan 021:** ordenação determinística — empates de score resolvem por `chunk_id ASC` (antes, caíam na ordem aleatória do `HashMap` e duas queries idênticas podiam devolver ordens diferentes); propriedades formalizadas em `tests/rrf_proptest.rs` (determinismo, união preservada, monotonicidade de rank, limites de score). **plan 023:** `rrf_score` público — reusado pelo server na fusão RRF dos sumários RLM (`summary_search`, espaço C).
