...
fn test_get_project_summary() {
    ...
    storage
        ...
        .unwrap();
    storage
        .insert_summary(buffer_id, "project", "project", None, Some("ph"), 1.0, None, None)
        .unwrap();
...
fn test_get_summary_by_source_hash() {
    ...
    let buffer_id = make_buffer(&storage);
    storage
        .insert_summary(buffer_id, "content", "module", None, Some("abc"), 0.8, None, None)
        .unwrap();

    let found = storage.get_summary_by_source_hash(buffer_id, "abc").unwrap();
...
fn test_source_chunk_ids_roundtrip() {
    ...
    let buffer_id = make_buffer(&storage);
    storage
        .insert_summary(buffer_id, "c", "file", Some(&[10, 20]), None, 0.1, Some(5), None)
        .unwrap();
...
fn test_search_summaries_by_buffer() {
    ...
    let hits = storage.search_summaries("authentication", buffer_id, 10).unwrap();
...
fn test_search_summaries_all() {
    ...
    storage
        .insert_summary(b1, "vector store indexing", "module", None, None, 0.9, None, None)
        .unwrap();
    storage
        .insert_summary(b2, "vector store retrieval", "project", None, None, 0.9, None, None)
        .unwrap();
...
fn test_get_summary_by_id() {
    ...
    let buffer_id = make_buffer(&storage);
    let id = storage
        .insert_summary(buffer_id, "by id lookup", "file", None, None, 0.9, None, None)
        .unwrap();
