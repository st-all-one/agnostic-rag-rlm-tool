...
mod tests {
    ...
    fn store_and_exact_hit() {
        ...
        let stored = storage.store_answer(&input).expect("store");
        assert!(stored.created);

        // Idempotent reuse on identical question.
        let again = storage.store_answer(&input).expect("store again");
    ...
    fn stale_and_delete() {
        ...
        assert_eq!(storage.delete_qa(stored.id).expect("delete"), 1);
        assert!(storage.get_qa_by_rowid(stored.id).expect("get").is_none());
    }
    ...
    fn lifecycle_marks_stale_by_hash() {
        ...
        let input = StoreAnswerInput {
            ...
            answer_text: "A".into(),
            source_chunk_ids: vec![],
            source_hashes: vec!["h1".into(), "h2".into()],
            model: None,
            tier_snapshot: None,
            token_count: 1,
        };
        storage.store_answer(&input).expect("store");
        let n = storage
    ...
    fn eviction_keeps_top_entries() {
        ...
        for i in 0..5 {
            let input = StoreAnswerInput {
                ...
                question_hash: question_hash(&format!("Q{i}")),
                answer_text: "A".into(),
                source_chunk_ids: vec![],
                source_hashes: vec!["h1".into()],
                model: None,
        ...
        let q0 = storage
            ...
            .expect("get")
            .expect("some");
        storage.touch_qa(q0.id).expect("touch");
        storage.touch_qa(q0.id).expect("touch");

        let removed = storage.evict_qa("p1", 3, 1_000_000).expect("evict");
        assert_eq!(removed, 2);
        assert!(
            storage
                .get_cached_answer("p1", &question_hash("Q0"))
