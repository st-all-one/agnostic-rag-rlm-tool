...
impl TokenCounter {
    ...
    ///
    /// A real BPE tokenizer is expensive; this approximation is cheap and far more
    /// accurate than a plain word count: it assumes roughly 4 characters per token,
    /// then adds a small surcharge for ASCII punctuation (each punctuation char adds
    /// ~0.25 token) since punctuation is usually split into its own token. The result
    /// is `ceil(chars/4 + punctuation/4)`. This stays within ~15% of most English
