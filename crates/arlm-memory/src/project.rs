...
impl ProjectInfo {
    fn from_buffer(buffer: &arlm_storage::sqlite::buffers::Buffer) -> Self {
        Self {
            id: buffer.id,
    ...
    fn test_create_project() {
        ...
        let info = mgr
            .create(&CreateProjectOptions {
                ...
                path: PathBuf::from("/tmp/my-proj"),
            })
            .unwrap();

        assert_eq!(info.name, "my-proj");
        assert_eq!(info.path, PathBuf::from("/tmp/my-proj"));
        assert!(info.id > 0);
    }
