...
use arlm_core::qa_cache::{QaThresholds, resolve_plan};
use arlm_proto::proto::*;
use arlm_search::qa_cache::jaccard_similarity;
use arlm_storage::qa_cache as qa_store;
use arlm_storage::qa_cache::StoreAnswerInput;
...
/// Map hydrated `arlm_search::SearchResult` into the gRPC `SearchResult` shape.
...
pub async fn handle_invalidate_cache(
    ...
    let invalidated = store::blocking({
        ...
        move || arlm_storage::cache::invalidate_cache(&storage, p.as_deref())
    })
    .await
    .map_err(internal)?;
...
mod tests {
    ...
    async fn query_store_answer_hit_get_invalidate() {
        ...
        *g.metadata_mut() = auth.clone();
        ...
        let (_, admin_refresh) = arlm_storage::tokens::create_token(
            &storage,
            &NewToken {
                username: "admin1".into(),
                role: Role::Admin,
        ...
        let (admin_session, _, _, _) =
            arlm_storage::tokens::create_session(&storage, &admin_refresh).expect("admin session");
