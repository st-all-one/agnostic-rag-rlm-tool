...
- `src/grpc/runs/engine.rs::spawn_engine` persiste o run e dispara
  `arlm_core::run_rlm_engine_with_events` em `tokio::spawn`, faz o bridge de
  eventos (`arlm_core::EventBus`) para o `EventHub` e persiste o resultado
...
- Criado `docker-compose.server.yml` (porta `50051`, `command: ["up"]`,
  `dockerfile: Dockerfile.server`). O root `docker-compose.yml` pertence ao CLI `arlm`.
...
- Subcomando `arlm-server status` (cliente gRPC `GetServerStatus`) valida o
  `HEALTHCHECK CMD arlm-server status` do `Dockerfile.server`.
...
## Verificação
- `cargo check -p arlm-server` → 0 erros, 0 warnings (do crate).
- `cargo test -p arlm-server` → 22 testes passando (6 store + 9 summarizer + 7 indexing).
