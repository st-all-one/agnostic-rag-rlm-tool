...
- **Parte A** — `Dockerfile` único (Ollama + arlm-server + modelo bakeado).
- **Parte B** — correções no `arlm-server` (onde, o quê, por quê) para liberar
...
1. **Um único container.** Ollama e arlm-server no mesmo `PID namespace`;
...
4. **Paralelismo em duas camadas** que precisam casar:
   ...
   - arlm-server: `buffer_unordered(N)` disparando lotes de `embed_batch`.
...
docker/entrypoint.sh      # sobe ollama, cria modelo, sobe arlm-server
```
...
COPY crates ./crates
# (se o workspace tiver mais de um binario, restringe com --bin arlm-server)
RUN cargo build --release --bin arlm-server

# ---------- Runtime: Ollama + arlm-server ----------
...
# Binario do servidor (confirmar nome em crates/arlm-server/Cargo.toml [[bin]])
...
# /data/arlm SIM: e o indice que precisa persistir.
VOLUME ["/data/arlm"]
EXPOSE 11434 50051
...
ollama create arlm-embed -f /opt/arlm/Modelfile || \
...
**Regra de ouro:** `ARLM_INDEX_CONCURRENCY` (servidor) deve ser **≈**
...
## Parte B — Correções no arlm-server

### B.0 Diagnóstico atual (por que não paraleliza hoje)

`crates/arlm-server/src/grpc/index.rs` (Phase 2, linhas ~112-127):
...
`crates/arlm-server/src/grpc/index.rs:24`
```rust
...
`crates/arlm-server/src/grpc/search.rs:74`
```rust
...
| **B4** | `config.rs` / `state.rs` | Ler `ARLM_INDEX_CONCURRENCY` e `ARLM_EMBED_BATCH` (defaults 4 / 64) e repassar à Phase 2. | Tuning sem rebuild da imagem; casa com `OLLAMA_NUM_PARALLEL`. |
| **B5** | (herdado) | — | O **BM25 OR-fallback** já foi implementado no `arlm-search` (hybrid) e é herdado pelo servidor → sem ação. |
...
> `embed_batch` (trait `Embedder`, `crates/arlm-embedding/src/embedder/mod.rs:86`)
> já existe e o `OllamaEmbedder` o implementa. `futures` já é dependência do
> `arlm-server` (Cargo.toml). Adicionar `use futures::stream::{self, StreamExt};`.
...
Em `crates/arlm-server/src/config.rs` (`ServerConfig`) ou direto em `state.rs`,
expor:
```rust
pub index_concurrency: usize,   // default 4
pub embed_batch_size: usize,    // default 64
```
e ler de `ARLM_INDEX_CONCURRENCY` / `ARLM_EMBED_BATCH` no `ServerConfig::load()`.
...
1. **Build:** `docker build -t arlm-ollama .` (baixa `all-minilm` no build).
2. **Run:** `docker run --cpus=4 -p 50051:50051 -p 11434:11434 -v arlm-data:/data/arlm arlm-ollama`.
3. **Smoke:** `curl http://localhost:11434/api/tags` deve listar `arlm-embed`.
...
5. **Busca:** `arlm --project X query "..."` (ou cliente gRPC) retorna contexto
...
- `Dockerfile` (multistage, base `ollama/ollama`, binário `arlm-server`).
- `docker/Modelfile`, `docker/entrypoint.sh`.
- Correções `arlm-server`: **B1** (Phase 2 paralela), **B2** (`dimensions()`),
