# 2. CLI `arags` — Referência Completa
...
| `-f, --format <fmt>` | `text` para search/query; `path` para os demais | Formato de saída: `full_json`, `path`, `markdown`, `text`, `jsonl` |
...
> (seção 2.5) ou do env `ARAGS_SERVER_ADDR`.

### Formatos de saída

| Valor | Uso ideal | Conteúdo |
|-------|-----------|----------|
| `text` (**default** search/query) | consumo direto por LLM/agente | contexto renderizado como prompt: chunks verbatim + seções "RLM Summaries" e "Exploration Maps" quando houver |
...
| `--no-index` | Só cria o `.arags.toml` (mutuamente exclusivo com `--index`) |
...
| `--unregister` | — | Para o daemon e remove o registro (mutuamente exclusivo com `--register`) |
...
Contrato completo em `EXPLORATIONS.md`. Três subcomandos:
...
fire-and-forget: falha de rede não derruba a sessão. Com
`[exploration].require_review=true`, não-admins recebem `status=pending_review`.

#### `explore feedback` — dever cívico do consumidor

| Arg/Flag | Descrição |
|----------|-----------|
| `<EXPLORATION_ID>` | id retornado no `explore search` |
| `--confirm` | Você **verificou no código** o mecanismo descrito |
| `--contradict` | Encontrou evidência contrária (acumula até aposentar o mapa) |

```bash
arags explore feedback 018f5a... --confirm
```
...
## 2.4 Resiliência e conexão do cliente
...
## 2.5 Configuração do usuário (2 escopos, merge granular)
...
Continua em: [03-server-docker.md](03-server-docker.md)
