# 3. `arags-server` + Docker — Referência Completa

> O servidor é o **dono do estado**: plano de dados gRPC puro, **LLM-free**
> (nem transitive). Toda a configuração vive num único arquivo de host
...
## 3.1 Subcomandos do binário `arags-server`
...
| `arags-server admin <sub>` | CLI interno de administração (seção 3.3). |
...
| `ARAGS_EMBEDDER_MODEL_DIR` | `[embedder].model_dir` |
...
[embedder]
model_dir = "/models"             # model.safetensors + tokenizer.json
...
contradiction_limit = 3           # nº de --contradict p/ aposentar (0 = nunca)
...
require_review = false            # true => não-admins caem em pending_review
```

> **Não existe seção `[llm]`** no `server.toml` — o servidor é LLM-free por
> construção (o crate `arags-llm` nem está no grafo dele).

Referência comentada pronta para montar: `docker/server.toml`.

---

## 3.3 CLI admin dentro do container (`arags-server admin`)
...
|-----|---------|-----------|
| `ARAGS_BIN_URL` | *(vazio)* | URL de tarball `.tar.gz` musl com `arags-server`; definido, **pula a compilação** (build vira download+COPY). Pensado p/ assets de GitHub Release. |
...
```bash
docker run -d ... \
  -v /srv/models/minilm:/models:ro \
  -e ARAGS_EMBEDDER_MODEL_DIR=/models \
  arags-server
```

Sem pesos válidos o servidor degrada para **hash embedder** (busca só BM25/FTS;
sem semântica) — não crasha, mas perde metade da busca.

### Persistência e backup

Tudo relevante vive no volume `/data` (seção layout em [01-arquitetura.md]).
Backup consistente:
...
O WAL garante crash-safety; o checkpoint PASSIVE periódico
(`flush_interval_ms`) mantém o WAL pequeno.

### CI/release

`.github/workflows/release.yml` já aponta para `docker/Dockerfile` (contexto =
raiz do repo). Quando publicarem o asset musl, o step passa `ARAGS_BIN_URL`
apontando ao próprio asset — imagem passa a ser download + COPY.

---

## 3.5 RPCs disponíveis (para clientes próprios)
...
| Admin/status | `TriggerMaintenance` (admin), `GetServerStatus` |
...
Continua em: [04-boas-praticas.md](04-boas-praticas.md)
