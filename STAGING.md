...
- **`agnostic-rlm-rs-49d6`** (Critical, epic, ⛔ blocked) — *Consistência SQLite↔vetores*: reconciliação por cron com SQLite como fonte canônica. Guarda-chuva de confiabilidade dados↔usearch; precede/paralelo aos itens abaixo.
...
- **`agnostic-rlm-rs-36ae`** (High, task) — *Reconcile worker*: ticker (`maintenance.rs`, já iniciado em `lifecycle.rs`) varre `pending_vector` (marcado em `50ed`) e re-embutir do conteúdo canônico nos espaços usearch; métricas/logs de gap. Padrões §1.4/§1.6/§1.3.
- **`agnostic-rlm-rs-620d`** (High, task) — *Bootstrap/rebuild*: na inicialização, reconstrói os 4 espaços vetoriais a partir do SQLite quando divergentes; flush vira otimização, não requisito. Arquivos: `state.rs`, `VectorSpaceStore`. Padrões §1.4/§1.6/§1.3.
...
- **`agnostic-rlm-rs-a5d7`** (High, task) — *Design + config de quorum* no `server.toml` (n=3, `quorum_sim_threshold=0.85`, estratégia de fusão, `strikes_limit`) + tabela `submissions` (candidate/accepted/rejected). Padrões §1.5/§1.4.
- **`agnostic-rlm-rs-6d97`** (High, feature, ⛔ blocked) — *Multi-atribuição RLM*: job vai a N voluntários (leases independentes mesmo em generation-group), recebe candidatos e decide por quorum cosseno com fusão das concordantes. Padrões §1.6/§1.4.
- **`agnostic-rlm-rs-64af`** (High, epic, ⛔ blocked) — *Quorum matemático (BFT leve)* para submissões de non-admins em RLM e Explorations.
- **`agnostic-rlm-rs-f486`** (Medium, feature, ⛔ blocked) — *Trust score de voluntário*: strikes por divergência/rejeição, ranking na seleção de claimers, reatribuição automática.
- **`agnostic-rlm-rs-f5f3`** (High, feature, ⛔ blocked) — *Remover `explore feedback` da superfície pública* (risco sybil por IA); aprovação vira modelo RLM (admin auto-aprova, non-admin → quorum).
- **`agnostic-rlm-rs-e89e`** (Medium, feature) — *Explorations non-admin*: modo de validação configurável — QUORUM como default, `require_review` opcional.
- **`agnostic-rlm-rs-d172`** (Medium, feature) — *Re-digest de QA via fila com lease*: job `pending_qa` com `preferred_user` → fallback voluntários → timeout 300s devolve ao queue.
